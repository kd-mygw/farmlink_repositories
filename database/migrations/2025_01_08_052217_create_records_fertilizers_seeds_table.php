...
return new class extends Migration
{
    ...
    {
        Schema::create('records_fertilizers_seeds', function (Blueprint $table) {
            $table->id();
            $table->timestamps();

            $table->date('date'); // 日付
            $table->unsignedBigInteger('cropping_id'); // 作付ID
            $table->string('seeds_name'); // 種苗名
            ...
            $table->foreign('seeds_name')->references('name')->on('fields')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('records_fertilizers_seeds');
    }
};
